-- https://raw.githubusercontent.com/pcxo/cxos-admin/refs/heads/main/admin
-- i didn't make this, cxo did (https://github.com/pcxo)
...
local success, result = pcall(function() -- country code loader (for plr info)
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			...
			function giveshirt(v, id)
				if v and v.Character then
					for i, c in pairs(v.Character:children()) do
						if c:IsA("Shirt") then
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		...
		if arg and arg ~= "" and arg ~= "n" and arg ~= "t" then
				...
				if z:IsA("BasePart") then
					if not parts[z.CFrame] then
						...
						if cloned.Parent then
							local parentname = cloned.Parent:GetFullName():gsub("^".. BackupTabby:GetFullName(), Tabby:GetFullName())
							local parent = Tabby:FindFirstChild(parentname, true)
							if parent then
							...
							notfi(sender, "cxo's admin", "cloned.Parent is nil for: ".. cloned.Name)
						end
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			...
			if fail then
				notfi(sender, "cxo's admin", "tools failed to load: ".. fail)
			end
		end
	end
})
			...
			task.spawn(function()
				...
				elseif a:lower() == "on" or a:lower() == "true" then
					if not workspace:FindFirstChild("cxo's admin vis (".. plr.Name ..")") then
						...
						create(p, f, cfgs.amount, cfgs.sizeX, cfgs.sizeY, cfgs.sizeZ)
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		task.spawn(function()
			...
			if #argt <= 0 then
				notfi(sender, "cxo's admin", "Invalid user! command example: ".. prefix .."hide ripcxo")
				return
			end
			...
			local function hideplr(v)
				...
				scr:WaitForChild("Exec").Value = string.format([[
					...
					game.Players:FindFirstChild(%s):Destroy()

				]], v)

				for i,e in pairs(game.Players:GetPlayers()) do
					if e.PlayerGui then
						if e.PlayerGui:FindFirstChild("GODMODE") then return end
						loa.Parent = e.PlayerGui
					else
						if e.Character then
							if e.Character:FindFirstChild("GODMODE") then return end
							loa.Parent = e.Character
						end
					end
				end
			end
...
function gmodlistener()
	...
	remote.OnServerEvent:Connect(function(player, action, target)
		...
		if not table.find(gmodded, player.UserId) then
			notfi(player, "cxo's admin", "This remote is protected.")
			return
		end
...
function create(plr, parent, amount, sizeX, sizeY, sizeZ)
	local cfgs = plrcfgs[plr.Name]
	for i = 1, amount do
...
function updatec(plr)
	...
	if cfgs.mode == "rainbow" then
		...
		for i, part in pairs(parts) do
			if part:FindFirstChild("maxwell") then
				part.maxwell:Destroy()
			end
			part.Material = cfgs.material
			part.Reflectance = cfgs.reflectance
			part.Color = colour3
		end
	elseif cfgs.mode == "redblue" then
		cfgs.colour = not cfgs.colour
		local colour3 = cfgs.colour and Color3.fromRGB(0, 0, 255) or Color3.fromRGB(200, 0, 0)
		for i, part in pairs(parts) do
			if part:FindFirstChild("maxwell") then
				part.maxwell:Destroy()
			end
